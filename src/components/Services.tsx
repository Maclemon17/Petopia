...
const Services = () => {
    return (
        <section className="bg-[#f5eee6] py-16 px-6 lg:px-24" id="services">
            ...
            <p className="text-center text-[#8d6748] max-w-xl mx-auto mb-12">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 ">
                {
                    petServices.map((service, index) => (
                        <div key={index} className="bg-white p-6 rounded-xl shadow-md hover:shadow-xl transition duration-300 border border-[#e6d3cd]" >
                            <div className="text-[#a8522d] mb-4 cursor-pointer">{service.icon}</div>
                            <div className="text-xl font-semibold text-[#6b4226] mb-2">{service.title}</div>
                            <div className="text-[#5c4433]">{service.desc}</div>
                        </div>
                    ))
                }
            </div>
